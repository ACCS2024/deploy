...
#### php.sh
- **配置文件修改**: 所有 sed 操作使用 `.original` 备份原始文件（只备份一次）
- **幂等性匹配**: 使用 `^;*` 同时匹配注释和非注释行，避免重复修改
  - `sed "s/^;*opcache.enable=.*/opcache.enable=1/"` - 无论是 `;opcache.enable` 还是 `opcache.enable` 都能正确处理
  - 第一次运行：移除注释并设置值
  - 第二次运行：直接更新值（不会追加）
- **环境变量转义**: 正确转义 `/tmp` 中的斜杠
